...
> In this article, we explain how to use Terraform's **taint** and **untaint** commands to manage resources recreation effectively.
>* These commands are especially useful when a resource fails during creation or when manual changes occur that necessitate a fresh deployment.


## Overview
Terraform marks a resource as **tainted** when it encounters errors during creation, such as a failed provisioner command. 
- **A tainted resource is scheduled for replacement during the next apply.**
- Conversely, you can use the **untaint** command to clear this status and prevent a replacement.
...
Consider a scenario when an AWS EC2 instance is provisoned using a local provisioner to store its public IP address in a file.

If the provisioner command fails—perhaps because the file path is incorrect—the resource is marked as tainted, triggering its replacement on the next apply.
...
There are situations where you might want to deliberately force a resource rebuild.

- For example, if manual changes—such as updating the Nginx version—were made on an AWS instance, you can efficiently trigger the recreation of that resource without performing a full destroy and apply cycle.
...
If you later decide that a resource should not be replaced, you can remove its tainted state by using the untaint command. 

-   This prevents Terraform from destroying and recreating the resource during the next apply.
